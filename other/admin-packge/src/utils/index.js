...
//天
const activityTime = function(date) {
    console.log('时间戳'+date)
    function two(n) {
        return n < 10 ?  n : '' + n;
    }
    let d = Number(two(Math.floor(date / 86400)))
    let h = Number(two(Math.floor(date % 86400 / 3600)))
    let s = Number(two(Math.floor(date % 3600 / 60)))
    let m = Number(two(Math.floor(date % 60)))
    // return this.time == null ? `正在计算时间...` : this.time > 0 ? `结束倒计时：${Number(d*24)+Number(h)}时${s}分${m}秒` : `活动已结束`;
    return (h > 0 || s > 0 || m > 0) && d == 0 ? 1 : d;
};
...
export default { get, post, openTime, downImg, baseUrl, qqMapInfo, FmtTime, getDateTime, activityTime, getPriceType, phModel, loginModel }
