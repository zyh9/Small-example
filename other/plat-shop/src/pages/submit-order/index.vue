<template>
  <div class="submit_order" v-if="block">
      ...
      <div class="con_order_info">
        ...
        <ul class="con_order_list">
          <li v-for="(v,i) in cartListItem" :key="i" class="con_list_item">
            ...
            <div class="sum">
              <p class="price">
                <i class="icon_discount_text" v-if="v.GoodsType==-1 || v.GoodType==-1"></i><span>¥</span>{{v.TotalMoney}}
              </p>
              <p class="original_price" v-if="v.GoodsType==-1 || v.GoodType==-1">¥{{v.sumPrice}}</p>
            </div>
          </li>
        </ul>
    ...
    <div class="distribution_card" :class="{distribution_card_active:isActive}">
      <div class="distribution_card_item">
        <div class="distribution_title">
          <p>配送方式+配送时长</p>
          <div @click="isActive = false" class="title_img">
    ...
    <div class="pay_card" :class="{pay_card_active:payMask}">
      ...
      <div class="pay_ok" @click="payMask=false">确定</div>
    </div>
  ...
  export default {
    ...
    onShow() {
      ...
      if (this.$mp.query.orderId) {
        ...
        //针对选中的商品提交订单
        let cartListInfo = cartItem.length ? cartItem[0].cartList : [];
        this.cartListItem = cartListInfo.filter(e => e.check == true);
    ...
    methods: {
      ...
      createOrder() {
        //生成订单号
        this.util.post({
          url: '/api/Customer/Order/SubmitOrder',
          data: {
