...
### 3.Express 和 koa 有什么关系，有什么区别？

		koa 是由 Express 原班人马打造的，致力于成为一个更小、更富有表现力、更健壮的 Web 框架，都是基于nodejs平台的搭建node服务器的框架
		
		koa是一个比express更精简，使用node新特性的中间件框架，相比之前express就是一个庞大的框架，它有足够的扩展和中间件
		
		koa支持es6语法，另外可以用同步的方式写异步

### 4.js的设计模式知道那些

		单体模式、工厂模式、单例模式、观察者模式
		策略模式、模板模式、代理模式、外观模式...
